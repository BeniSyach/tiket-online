...
<div class="w-full bg-gray-100">
    <div class="max-w-7xl mx-auto flex flex-col md:flex-row w-full justify-between py-5 px-6 border-b-2">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-2 mt-2">
            <div class="md:px-2">
                ...
                <div><a href="{{ route('page.show', ['page' => 'aboutus']) }}"
                        class="hover:text-blue-500">{{ __('website.About Us') }}</a></div>
            ...
            <div class="md:px-2">
                ...
                <div>
                    <a href="{{ route('page.show', ['page' => 'term-of-service']) }}"
                        class="hover:text-blue-500">{{ __('website.Terms of service') }}</a>
                </div>
                <div><a href="{{ route('page.show', ['page' => 'privacy-policy']) }}"
                        class="hover:text-blue-500">{{ __('website.Privacy Policy') }}</a></div>
                <div><a href="{{ route('page.show', ['page' => 'cookiepolicy']) }}"
                        class="hover:text-blue-500">{{ __('website.Cookie Policy') }}</a></div>
                <div><a href="{{ route('page.show', ['page' => 'refundpolicy']) }}"
                        class="hover:text-blue-500">{{ __('website.Refund Policy') }}</a></div>
                <div><a href="{{ route('page.show', ['page' => 'disclaimer']) }}"
                        class="hover:text-blue-500">{{ __('website.Disclaimer') }}</a></div>
            </div>
            <div class="md:px-2">
                <div class="text-lg md:text-xl font-bold pb-2">{{ __('website.Products') }}</div>
                <div>
                    <a href="{{ route('tour-packages.index') }}"
                        class="hover:text-blue-500">{{ __('website.wisata') }}</a>
                </div>
                <div>
                    <a href="{{ route('car.index') }}" class="hover:text-blue-500">{{ __('website.Cari Parkir') }}</a>
                </div>
                {{-- <div>
    ...
    <div class="max-w-7xl mx-auto flex flex-col md:flex-row w-full justify-between items-center py-5 px-6">
        ...
        <div class="py-2">
            &copy; {{ now()->format('Y') }} {{ $setting->getValue('G_SITE_NAME') }}.
            {{ __('website.All Rights Reserved') }}.
        </div>
    </div>
</div>
