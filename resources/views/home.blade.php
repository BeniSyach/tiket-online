...
@section('content')
                ...
                @foreach ($setting->getSlides() as $slide)
                    ...
                    <div class="hidden brightness-90 duration-700 ease-in-out" data-carousel-item>
                        <span
                            ...
                            Slide</span>
                        <img src="{{ asset($slide) }}" loading="lazy"
    ...
    @endforeach
            ...
            <div class="absolute z-40 top-14 lg:top-16 left-1/2 -translate-x-1/2 w-full lg:w-2/3 mx-auto h-60 lg:px-0 px-4">
                <div class="text-center md:text-left text-5xl font-extrabold text-white outlined-text">
                    {{ $setting->getValue('G_SITE_WELCOME') }}</div>
                <div class="text-center md:text-left text-xl font-light text-white text-opacity-80">
                    ...
                    <ul class="flex flex-wrap text-sm font-medium text-center text-gray-500">
                        <li class="">
                            <a href="#" aria-current="page"
                                class="inline-block p-3 font-bold bg-blue-600 text-white rounded-t-lg active dark:bg-gray-800 dark:text-blue-500">{{ __('website.Fastboat') }}</a>
                        </li>
                        <li class="">
                            <a href="{{ route('car.index') }}"
                                class="inline-block p-3 font-bold rounded-t-lg bg-gray-100 hover:text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-800 dark:hover:text-gray-300">{{ __('website.Car Rentals') }}</a>
                        </li>
                    </ul>
                </div>
            ...
            <div class="flex flex-row justify-between">
                ...
                <div class="see-more-container mt-1">
                    <a href="{{ route('tour-packages.index') }}"
                        ...
                        {{ __('website.See More') }}
                        <svg aria-hidden="true" class="w-5 h-4 ml-1" fill="currentColor" viewBox="0 0 20 20"
                    ...
                    <div
                        ...
                        <div class="p-5">
                            <a href="{{ route('tour-packages.show', $package) }}">
                                <h5 class="mb-1 tracking-tight font-bold text-gray-900 line-clamp-1 dark:text-white">
                                    {{ $package->title }}</h5>
                            </a>
                            <span
                                class="mb-3 py-1 px-2 text-xs font-bold rounded bg-gray-200 text-gray-700 dark:text-gray-400">{{ __('website.Instant Confirmation') }}</span>
                            <div class="pt-5">
                                {{ __('website.From') }}
                                <span class="text-base font-bold">Rp.
        ...
        <div class="flex flex-col">
            <div class="flex flex-row justify-between">
                <div class="flex flex-col">
                    ...
                    <div class="text-gray-400">{{ __('website.We have pictures based on location that they upload') }}.
                    </div>
                </div>
                <div class="see-more-container mt-1">
            ...
            <div class="px-2 pt-2 grid grid-cols-1 gap-2 md:gap-4 md:grid-cols-3">
                @foreach ($images as $image)
                    <div
                        class="{{ $image->show_on == 1 ? 'md:col-span-2 md:row-span-2' : '' }} transition-all duration-300 cursor-pointer filter grayscale hover:grayscale-0 scale-100 hover:scale-105 hover:z-10">
                        <img class="object-fill w-full {{ $image->show_on == 1 ? 'h-full' : 'h-96' }} rounded-lg"
                            src="{{ $image->path_url }}" loading="lazy" alt="{{ $image->name }}" />
                        <div
                            class="absolute px-4 text-xl {{ $image->show_on == 1 ? 'md:text-4xl' : 'md:text-2xl' }} font-bold text-white bottom-10 md:left-10">
                            <p>{{ $image->name }}</p>
                        </div>
                    </div>
                @endforeach
    ...
    <div class="w-full max-w-7xl mx-auto px-2">
        <div class="flex flex-col w-full mx-auto pt-10 pb-5 text-center md:text-left">
            <div class="flex flex-row justify-between">
                <div class="text-3xl font-bold pb-1">{{ __('website.Recent Updates') }}</div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4 py-2 px-2">
                @foreach ($posts as $post)
                    <div
                        class="hover:-translate-y-2 ease-in duration-150 shadow bg-white border border-gray-200 rounded-lg dark:bg-gray-800 dark:border-gray-700">
                        <a href="{{ route('blog.post', $post) }}">
                            <img class="rounded-t-lg h-48 w-full" src="{{ asset($post->cover_image) }}" loading="lazy"
                                alt="{{ $post->title }}" />
                        </a>
                        <div class="p-3 md:p-5 space-y-1">
                            <a href="{{ route('blog.post', $post) }}">
                                <h4 class="font-bold text-xl text-gray-900 dark:text-white line-clamp-1">
                                    {{ $post->title }}</h4>
                            </a>
                            ...
                            <div>
                                @foreach ($post->tags as $tag)
                                    <span
                                        class="py-1 px-2 text-xs font-bold rounded bg-gray-200 text-gray-700 dark:text-gray-400">{{ $tag->name }}</span>
                                @endforeach
                            </div>
                        </div>
                    </div>
    ...
    <div class="w-full max-w-7xl mx-auto px-2 py-4">
        <div class="grid grid-cols-1 md:grid-cols-2">
            <div class="flex flex-col px-2">
                <div class="text-3xl text-center md:text-left font-bold md:text-4xl md:font-semibold pb-6">
                    {{ __('website.Frequently Asked Questions') }}</div>
                <div class="relative md:w-4/5">
                    ...
                    <form action="{{ route('page.faq') }}">
                        <input type="text" id="simple-search"
                            ...
                            placeholder="{{ __('website.Search Topics') }}" required name="q" />
                    </form>
                </div>
            </div>
            <div class="flex flex-col text-xl" id="accordion-collapse" data-accordion="collapse">
                ...
                @foreach ($faqs as $index => $faq)
                    <button type="button" class="w-full border-b-2 inline-flex justify-between py-4 px-1 items-center"
                        id="accordion-collapse-heading-{{ $index }}"
                        data-accordion-target="#accordion-collapse-body-{{ $index }}" aria-expanded="false"
                        aria-controls="accordion-collapse-body-1">
                        <p class="font-bold text-lg md:text-xl">{{ $faq->question }}
                        <p>
